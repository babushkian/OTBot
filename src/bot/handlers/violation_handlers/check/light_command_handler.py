...
from .states import LightViolationCheckStates
...
@router.callback_query(ViolationsFactory.filter(), LightViolationCheckStates.start)
async def handle_violation_review(
    ...
    await state.set_state(LightViolationCheckStates.review)
...
@router.callback_query(ViolationsActionFactory.filter(F.action == "activate"), LightViolationCheckStates.review)
async def handle_violation_activate(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(LightViolationCheckStates.activate)
    await callback.answer("Выбрано действие утверждения.")


@router.message(LightViolationCheckStates.activate, F.text.in_(["✅ Да", "❌ Нет"]))
...
@router.callback_query(ViolationsActionFactory.filter(F.action == "reject"), LightViolationCheckStates.review)
async def handle_violation_reject(
    ...
    await state.set_state(LightViolationCheckStates.reject)
    await callback.answer("Выбрано действие отклонения.")


@router.message(LightViolationCheckStates.reject, F.text.in_(["✅ Да", "❌ Нет"]))
